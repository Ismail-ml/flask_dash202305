...
def register_callback(dashapp):
    ...
    @dashapp.callback(Output('Data Traffic', 'figure'),
                      ...
                      Output('table_elave_et','children'),
...
#)
        ...
        ######################################################## BURDAN SILLL ##########################################################
        needed = dt.strptime(dt.strftime(dt.now(), '%d.%m.%y %H:00'), '%d.%m.%y %H:00')
        hh = pd.date_range(end=needed, periods=24, freq='1H')
        dates=set([dt.strftime(i,'%Y-%m-%d') for i in hh])
        bkc_alarms=[]
        azf_alarms=[]
        for d in dates:
            bkc_alarms.append(pd.read_hdf('/disk2/support_files/archive/anomality/'+d+'_anomalies.h5','it',where='Date>hh[0]'))
            azf_alarms.append(pd.read_hdf('/disk2/support_files/archive/anomality/'+d+'_anomalies.h5','it_azf',where='Date>hh[0]'))
        bkc_alarms=pd.concat(bkc_alarms)
        bkc_alarms.insert(1,'MNO','BKC')
        azf_alarms=pd.concat(azf_alarms)
        azf_alarms.insert(1,'MNO','AZF')
        filted2=pd.concat([bkc_alarms,azf_alarms])
        filted2[['value','up_threshold','down_threshold']]=round(filted2[['value','up_threshold','down_threshold']],2)
        filted2['Date'] = [dt.strftime(filted2['Date'].iloc[i].to_pydatetime(), '%d.%m.%Y %H:%M') for i in
                          range(len(filted2['Date']))]
        filted2.loc[filted2['down_threshold']<0,'down_threshold']=0
        tt=dash_table.DataTable(id='live-table',
                                        columns=[{'name': i, 'id': i} for i in filted2.columns],
                                        data=filted2.to_dict('records'),style_cell={'textAlign': 'center',
                                                                                ...
                                                                                #'width':str(100 / (len(filted2.columns))) + '%'},
                    style_header={
...
page_action="native",
        ...
        return fig,figure, figure2,tt#, fig2, fig3#, figure_ses, nese3
