...
def load_data():
    df = pd.read_csv(r'/home/ismayil/flask_dash/support_files/worst_sites_hourly.csv')
...
def register_callback(dashapp):
    ...
    def plot_map_graph(selected_dropdown_value, cell, period,n):
        ...
        df_sub = df[(df['KPI name'] == selected_dropdown_value) & (df['Date'] == dates[period2])][:15]
        ...
        figure.update_layout(title=
                             go.layout.Title(text='<b>' + selected_dropdown_value + ' values for ' + str(
                                 dt.strftime(dt.utcfromtimestamp(dates[period2].tolist() / 1e9),
                                             '%d.%m.%Y %H:%M')) + '</b>',
                                             font=dict(
        ...
        df_sub['Date'] = [dt.strftime(df_sub['Date'].iloc[i].to_pydatetime(), '%d.%m.%Y %H:%M') for i in
