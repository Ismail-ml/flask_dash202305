...
def read_csv(filename):
    ...
    try:
    	...
    	df=df[df['AlarmName'].str.contains('POWER|Power|power|Rectifier|RECTIFIER|PHASE|Phase|GENERATOR|Generator')]
    	k+=1
...
file_list=glob.glob("/mnt/raw_counters/Corporate Folder/CTO/Alarm_LOG/2022*.zip")
print(len(file_list))
def main(i,j):
    # set up your pool
    c1=time.time()
...
while True:
    ...
    if (len(new)%10==0) and (len(new)>0):
    	pd.concat(new).drop_duplicates().to_csv('u2000_alarms'+str(j)+'_2022_without_duplicates.csv')
    	new=[]
#print(len(new),'h len')
#print(pd.concat(new).shape)

pd.concat(new).to_csv('u2000_alarms_2022.csv')
pd.concat(new).drop_duplicates().to_csv('u2000_alarms_2022_without_duplicates.csv')
