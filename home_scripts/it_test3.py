...
def proccess_excel(diddd):
        try:
            k1=pd.DataFrame(xmlDict['measCollecFile']['measData'][diddd]['measInfo']).T
            if k1.columns[0]==0:
                for i in k1.columns:
                    ...
                    h.insert(2,'LocalDn',xmlDict['measCollecFile']['measData'][diddd]['managedElement']['@localDn'])
                    h.insert(3,'UserLabel',xmlDict['measCollecFile']['measData'][diddd]['managedElement']['@userLabel'])
                ...
                if k1.loc['@measInfoId','measResults'] in dict.keys():
...
print('len of files = ',len(glob.glob('IT/*.xml')))
for k in glob.glob('IT/*.xml'):
    xml_data = open(k, 'r').read()  # Read data
    xmlDict = xmltodict.parse(xml_data)  # Parse XML
    print(k)

    with cf.ThreadPoolExecutor() as executor:
            executor.map(proccess_excel,range(len(xmlDict['measCollecFile']['measData'])))
...
for j in f.keys():
    ...
    for i in a['Date'].unique():
        ...
        #file_name = datetime.datetime.strftime(datetime.datetime.utcfromtimestamp(i.astype(datetime.datetime)/1e9), "%B_%Y")
            ...
            a.loc[a['Date']==i].to_hdf(r'/home/qarftp/' + file_name2 + '.h5', j , append=True,
