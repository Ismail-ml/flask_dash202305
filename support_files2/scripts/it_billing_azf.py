...
os.chdir('/home/ismayil/flask_dash/data/it/AZF')
for i in os.listdir():
...
for j in f.keys():
    ...
    if ('DCC' not in j) & ('Offline' not in j) & ('Forward' not in j): 
        if os.path.exists('/disk2/support_files/archive/it_trend_files/'+j+'_azf.csv'):
            a.to_csv('/disk2/support_files/archive/it_trend_files/'+j+'_azf.csv',index=False,header=False,mode='a')
        else:
            a.to_csv('/disk2/support_files/archive/it_trend_files/'+j+'_azf.csv',index=False,mode='a')
    #if j=='OSRuntime':
    #    if os.path.exists('/disk2/support_files/archive/OSRuntime_azf.csv'):
    #        a.to_csv('/disk2/support_files/archive/OSRuntime_azf.csv',index=False,header=False,mode='a')
    #    else:
    #        a.to_csv('/disk2/support_files/archive/OSRuntime_azf.csv',index=False,mode='a')
    ...
    for i in a['Date'].dt.date.unique():
        ...
        #file_name = datetime.datetime.strftime(datetime.datetime.utcfromtimestamp(i.astype(datetime.datetime)/1e9), "%B_%Y")
            ...
            a.loc[a['Date'].dt.date==i].to_hdf(r'/disk2/support_files/archive/it/azf_billing_' + file_name2 + '.h5', j , append=True,
...
final_all = []
not_evaluated = []
for k in ['hwBillingDCCMsgNumber','hwBillingDCCSessionNumber','OSRuntime', \
'persistRunningTime','CPU','LogicalCPU','Memory','VirtualMemory','hwBillingDCCSeviceCallNumber','hwForwardDiamMsgStat','hwOfflineStat'\
    ...
    try:
        ...
        for a in dates:
            ...
            if (k=='hwBillingDCCMsgNumber') | (k=='hwBillingDCCSessionNumber') | (k=='hwBillingDCCSeviceCallNumber') | (k=='hwForwardDiamMsgStat') | (k=='hwOfflineStat'):
            ...
            if k not in ['CPU','LogicalCPU','Memory','VirtualMemory','serviceDatabaseInf','Storage']:
                for i in set(files):
                    try:
                        d.append(pd.read_hdf('azf_billing_'+i+'.h5',k,where='Date in hh'))
                    except Exception as e:
                        print(e)
                        continue
            else:
            ...
            if k=='hwBillingDCCMsgNumber':
                ...
                dcc_sc.iloc[:,5:-1]=dcc_sc.iloc[:,5:-1].astype(float)
                dcc_sc['hour']=dcc_sc['Date'].dt.hour
                dcc_sc['minute']=dcc_sc['Date'].dt.minute
                ...
                dcc_df.iloc[:,5:-1]=dcc_df.iloc[:,5:-1].astype(float)
                dcc_df['hour']=dcc_df['Date'].dt.hour
                dcc_df['minute']=dcc_df['Date'].dt.minute
                ...
                dcc_off.iloc[:,6:-1]=dcc_off.iloc[:,6:-1].astype(float)
                dcc_off['hour']=dcc_off['Date'].dt.hour
                dcc_off['minute']=dcc_off['Date'].dt.minute
...
########################
    final.to_csv('/home/ismayil/flask_dash/support_files/anomality_detection/it_anomalies_azf.csv', index=False)
    ...
    if os.path.exists('/home/ismayil/flask_dash/support_files/anomality_detection/it_ongoing_anomalies_azf.csv'):
        final['hwEventType']=final['hwEventType'].astype(str)
        anomalies = pd.read_csv('/home/ismayil/flask_dash/support_files/anomality_detection/it_ongoing_anomalies_azf.csv')
    ...
    anomalies.to_csv('/home/ismayil/flask_dash/support_files/anomality_detection/it_ongoing_anomalies_azf.csv', index=False)
    anomalies[['Date','file','value']].groupby(['Date','file'],as_index=False).count().to_csv('/disk2/support_files/archive/azf_it_anom_trend.csv',mode='a',
    ...
    for d in anomalies['Date'].dt.date.unique():
        ...
        anomalies.loc[anomalies['Date'].dt.date==d].to_hdf(
            '/disk2/support_files/archive/anomality/' + file_name2 + '_anomalies.h5', '/it_azf',
