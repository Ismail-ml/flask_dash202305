...
def run(path1,path2,tracker):
    try:
        ...
        for dirpath,dirname,filenames in os.walk(path1):
            ...
            if len(all_files)>0:
                ...
                for filename in all_files:
                    ...
                    if (filename.replace('/home/ismayil/flask_dash/data/nokia/pool/','2G_') in existing_files.values) or ('03_37_00' in filename) or ('00_37_00' in filename) or ('01_37_00' in filename) or ('02_37_00' in filename) or ('CMG' in filename): continue
                    #if os.path.basename(filename) in existing_files: continue
                    #df = dd.read_csv(filename,sep=';',dtype=nokia_2G_1) change to previous value ############
                    files2 = zipfile.ZipFile(filename, 'r')
                    df = pd.read_csv(files2.open(files2.namelist()[0]), sep=';')
                    if ('_37_00__' not in filename):
                        df = df[df['PERIOD_START_TIME'].str.contains('02:|00:00:00|01:|23:')]
                    ...
                    'sdcch_drop_rate_den','cs_traffic_erl','ps_traffic_mb','tbf_est_sr_num','tbf_est_sr_den','tbf_drop_rate_num',
                           'tbf_drop_rate_den','cssr_num1','cssr_den1','cssr_num2','cssr_den2','cssr_num3','cssr_den3','AVE_IDLE_F_TCH_1 (c002006)','AVE_IDLE_F_TCH_2 (c002008)',
                'AVE_IDLE_F_TCH_3 (c002010)','AVE_IDLE_F_TCH_4 (c002012)','AVE_IDLE_F_TCH_5 (c002014)']]
        ...
        for dirpath,dirname,filenames in os.walk(path1):
            ...
            if len(all_files)>0:
                ...
                for filename in all_files:
                    #if os.path.basename(filename) in existing_files: continue
                    files2 = zipfile.ZipFile(filename, 'r')
                    if ('03_37_00' in filename) or ('CMG' in filename) or ('00_37_00' in filename) or ('01_37_00' in filename) or ('02_37_00' in filename): continue
                    df = pd.read_csv(files2.open(files2.namelist()[1]), sep=';',dtype=nokia_2G_2)
                    if ('_37_00__' not in filename):
                        df = df[df['PERIOD_START_TIME'].str.contains('02:|00:00:00|01:|23:')]
                    #df = dd.read_csv(filename,sep=';',dtype=nokia_2G_2) change to previous value ###########
                    #df = df[df['BSC name'] != 'ASBSC22']
                    li.append(df)
        ...
        nsn_agr['cell_avail_blck_den'].loc[(nsn_agr['cell_avail_num'] + nsn_agr['cell_avail_den']) == 0] = 3600
        nsn_agr['cell_avail_blck_num'] = 0
        nsn_agr['cell_avail_den'] = nsn_agr['cell_avail_den'] + nsn_agr['cell_avail_blck_den']
        nsn_agr['cell_avail_blck_num'].fillna(0, inplace=True)

        nsn_agr.rename(columns={'BTS name':'Cell_name','BSC name':'BSC_name','AVE_IDLE_F_TCH_1 (c002006)':'band_1','AVE_IDLE_F_TCH_2 (c002008)':'band_2',
                'AVE_IDLE_F_TCH_3 (c002010)':'band_3','AVE_IDLE_F_TCH_4 (c002012)':'band_4','AVE_IDLE_F_TCH_5 (c002014)':'band_5'},inplace=True)
        ...
        nsn_agr=nsn_agr[['Date','Vendor','BSC_name','Site_name','Cell_name','Region','call_block_rate_den',
               ...
               'tbf_est_sr_den', 'tbf_est_sr_num', 'tch_avail_den', 'tch_avail_num','band_1','band_2','band_3','band_4','band_5']]
        nsn_agr.iloc[:, 6:] = nsn_agr.iloc[:, 6:].astype(np.float64)
        nsn_agr.loc[:, 'cell_avail_blck_num'] = nsn_agr.loc[:, 'cell_avail_blck_num'].astype(np.int64)
        nsn_agr.loc[:, 'cell_avail_blck_den'] = nsn_agr.loc[:, 'cell_avail_blck_den'].astype(np.int64)
        ...
        for i in nsn_bsc['Date'].unique():
            ...
            try:
                a=pd.read_hdf('/disk2/support_files/archive/ran/'+file_name2 +'.h5', 'twoG/bsc', where='Vendor="Nokia" and Date=i')
            except:
                a=pd.DataFrame()
        ...
        #               min_itemsize={'BSC_name': 10, 'Vendor': 10, 'Region': 15})
        nsn_bsc.drop(columns=['band_1','band_2','band_3','band_4','band_5']).loc[nsn_bsc['Date'].isin(to_filter)].to_hdf(r'/disk2/support_files/archive/combined_bsc.h5', '/twoG',
                       append=True,
                       format='table', data_columns=['Date', 'BSC_name', 'Vendor', 'Region'], complevel=5,
                       min_itemsize={'BSC_name': 10, 'Vendor': 10, 'Region': 15})

        p=datetime.datetime.strftime(datetime.datetime.now(),'%d.%m.%Y_%H')
        #nsn_bsc.to_csv('/mnt/raw_counters/Corporate Folder/CTO/Technology Governance and Central Support/RAN QA/Daily/Radio_Main_KPIs/2G_nsn_'+p+'.csv',
        #                        index=False)



        df2=nsn_bsc.drop(columns=['band_1','band_2','band_3','band_4','band_5']).loc[nsn_bsc['Date'].isin(to_filter)]
        ...
        df2.to_csv('/disk2/support_files/atoti/bi/2G_nokia.csv',index=False) #' + str(datetime.datetime.now()) + '


        print(time.time() - c1)
        pd.DataFrame(glob.glob(path1 + '/*.zip')).replace({'/home/ismayil/flask_dash/data/nokia/pool/': '2G_'}, regex=True).to_csv(os.path.join('/home/ismayil/flask_dash/data', 'files.csv'), mode='a', header=None, index=False)
        for i in os.listdir('/home/ismayil/flask_dash/data/nokia/pool'):
            if ('Main' in i) & ('_37_00__' not in i):
                ...
                ('2G_' + existing_files.iloc[-1]).to_csv(os.path.join('/home/ismayil/flask_dash/data', 'files.csv'), mode='a', header=None, index=False)
        return 1
