...
def run(path1,path2,tracker):
    ...
    def proccess_excel2(diddd):
        ...
        for filename in all_files:
            ...
            'ul_ps_traf','1526728261','1526728259','rtwp']]
            df['Date']=pd.to_datetime(df['Result Time'], format='%Y-%m-%d %H:%M')
            df['Cell_name']=df['Object Name'].apply(lambda x: x[x.find('=',x.find('Cell Name'))+1:x.find(',',x.find('Cell Name'))])
            li.append(df)
    h=[]
    ...
    for i in hw_rnc['Date'].unique():
        ...
        hw_agr_finish.loc[hw_agr_finish['Date']==i].to_hdf(r'/disk2/support_files/archive/ran/' + file_name2 + '.h5', '/fourG', append=True,
                             format='table', data_columns=['Date', 'Site_name', 'Cell_name', 'Vendor', 'Region'],
                             complevel=5,
                             min_itemsize={'Site_name': 20, 'Cell_name': 20, 'Vendor': 10, 'Region': 15})
        hw_rnc.loc[hw_rnc['Date']==i].to_hdf(r'/disk2/support_files/archive/ran/' + file_name2 + '.h5', '/fourG/bsc', append=True,
                      format='table', data_columns=['Date', 'Vendor', 'Region'], complevel=5,
                      min_itemsize={'Vendor': 10, 'Region': 15})
    #hw_agr_finish.to_hdf(r'/disk2/support_files/archive/'+file_name+'.h5','/fourG',append=True,
    #           format='table',data_columns=['Date','Site_name','Cell_name','Vendor','Region'],complevel=5,
    #                min_itemsize={'Site_name':20,'Cell_name':20,'Vendor':10,'Region':15})
    #hw_rnc.to_hdf(r'/disk2/support_files/archive/'+file_name+'.h5','/fourG/bsc',append=True,
    #           format='table',data_columns=['Date','Vendor','Region'],complevel=5,
    #                min_itemsize={'Vendor':10,'Region':15})
    
    # burda ishleyirdi hw_rnc[['Date','Vendor','Region','volte_sr_num', 'volte_sr_den', 'volte_dr_num', 'volte_dr_den', 'volte_dl_ps_traf',
    ...
    #                                                  append=True,
    #                                                  format='table', data_columns=['Date', 'Vendor', 'Region'],
    #                                                  complevel=5,
    #                                                  min_itemsize={'Vendor': 10, 'Region': 15})
    #hw_rnc.drop(columns=['volte_sr_num','volte_sr_den','volte_dr_num','volte_dr_den','volte_dl_ps_traf',
    ...
    #hw_rnc.to_csv('/mnt/raw_counters/Corporate Folder/CTO/Technology Governance and Central Support/RAN QA/Daily/Radio_Main_KPIs/4G_hw_'+p+'.csv',
    #                        index=False)

    try:
        df4=hw_rnc[['Date','Region','cell_avail_den', 'cell_avail_num','dl_ps_traf','ul_ps_traf','cell_avail_blck_num',
        ...
        df4.to_csv('/disk2/support_files/atoti/bi/4G_huawei.csv',index=False) #' + str(datetime.datetime.now()) + '
