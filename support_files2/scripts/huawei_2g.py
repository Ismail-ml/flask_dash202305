...
import datetime
try:
    def run(path1,path2,path3,path4,tracker):
        ...
        def proccess_excel_ta_int(diddd):
            # existing_files = pd.read_csv(os.path.join(path2, 'files.txt'), sep=" ", header=None)
            all_files = glob.glob(diddd + "/*.csv")
            for filename in all_files:
                # if os.path.basename(filename) in existing_files: continue
                df = pd.read_csv(filename, skiprows=[1])
                df['band_1'] = df['1278270442']+df['1278270447']
                df['band_2'] = df['1278270443'] + df['1278270448']
                df['band_3'] = df['1278270444'] + df['1278270449']
                df['band_4'] = df['1278270445'] + df['1278270450']
                df['band_5'] = df['1278270446'] + df['1278270451']
                
                df = df[['Object Name', 'Result Time','band_1','band_2','band_3','band_4','band_5']]
                df['Date'] = pd.to_datetime(df['Result Time'], format='%Y-%m-%d %H:%M')
                df['BSC_name'] = df['Object Name'].apply(lambda x: x[:6])
                df['Cell_name'] = df['Object Name'].apply(lambda x: re.search(r'\w{4}\d{4}\w', x).group())
                df=df.groupby(['Date','BSC_name','Cell_name']).sum().reset_index()
                # df['Site_name']=df['Cell_name'].apply(lambda x: x[:8])
                li.append(df)

        h = []
        [h.append(r) for r, d, folder in os.walk(path4)]
        li = []

        with cf.ThreadPoolExecutor() as executor:
            executor.map(proccess_excel_ta_int, h)

        ta_int = pd.concat(li, axis=0, ignore_index=True, sort=False)
        print(time.time() - c1, 'ta_int finish')
        ...
        hw_agr2 = pd.merge(hw_agr1, ta_int, on=['Cell_name', 'BSC_name', 'Date'], how='left')
        hw_agr = pd.merge(hw_agr2, hw3, on=['Cell_name', 'BSC_name', 'Date'], how='left')
        print(time.time() - c1)
        ...
        # burda ishleyirdi hw_bsc.drop(columns=['band_1','band_2','band_3','band_4','band_5']).to_hdf(r'/disk2/support_files/archive/combined_bsc.h5', '/twoG',append=True,
        #              format='table', data_columns=['Date', 'BSC_name', 'Vendor', 'Region'], complevel=5,
        #              min_itemsize={'BSC_name': 10, 'Vendor': 10, 'Region': 15})
        
        p=datetime.datetime.strftime(datetime.datetime.now(),'%d.%m.%Y_%H')
        #hw_bsc.to_csv('/mnt/raw_counters/Corporate Folder/CTO/Technology Governance and Central Support/RAN QA/Daily/Radio_Main_KPIs/2G_hw_'+p+'.csv',
        #                        index=False)

        df2=hw_bsc.drop(columns=['band_1','band_2','band_3','band_4','band_5'])
        ...
        df2.to_csv('/disk2/support_files/atoti/bi/2G_huawei.csv',index=False) #' + str(datetime.datetime.now()) + '
