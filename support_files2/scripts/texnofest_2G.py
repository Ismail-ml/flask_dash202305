...
def run(path1,path2,tracker):
    try:
        ...
        if len(all_files)>0:
            ...
            for filename in all_files:
                try:
                    ...
                    if (filename.replace('/home/ismayil/flask_dash/data/nokia/','2G_') in existing_files.values): continue
        ...
        if os.path.exists('/disk2/support_files/archive/twoG_bsc.csv'):
            existing=pd.read_csv('/disk2/support_files/archive/twoG_bsc.csv')

        if os.path.exists('/disk2/support_files/archive/twoG.csv'):
            nsn_agr=nsn_agr[~nsn_agr['Date'].isin(existing['Date'].unique())]
            nsn_agr.to_csv('/disk2/support_files/archive/twoG.csv',header=False,index=False,mode='a')
        else:
            nsn_agr.to_csv('/disk2/support_files/archive/twoG.csv',index=False,mode='a')
        
        if os.path.exists('/disk2/support_files/archive/twoG_bsc.csv'):
            nsn_bsc=nsn_bsc[~nsn_bsc['Date'].isin(existing['Date'].unique())]
            nsn_bsc.to_csv('/disk2/support_files/archive/twoG_bsc.csv',header=False,index=False,mode='a')
        else:
            nsn_bsc.to_csv('/disk2/support_files/archive/twoG_bsc.csv',index=False,mode='a')

        pd.DataFrame(glob.glob(path1 + '/*.zip')).replace({'/home/ismayil/flask_dash/data/nokia/': '2G_'}, regex=True).\
