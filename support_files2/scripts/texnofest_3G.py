...
def run(path1,path2,tracker):
    ...
    if len(all_files)>0:
        ...
        for filename in all_files:
            try:
                if (filename.replace('/home/ismayil/flask_dash/data/nokia/','3G_') in existing_files.values): continue
    ...
    if os.path.exists('/disk2/support_files/archive/threeG_bsc.csv'):
        existing=pd.read_csv('/disk2/support_files/archive/threeG_bsc.csv')

    if os.path.exists('/disk2/support_files/archive/threeG.csv'):
        nsn_agr=nsn_agr[~nsn_agr['Date'].isin(existing['Date'].unique())]
        nsn_agr.to_csv('/disk2/support_files/archive/threeG.csv',header=False,index=False,mode='a')
    else:
        nsn_agr.to_csv('/disk2/support_files/archive/threeG.csv',index=False,mode='a')

    if os.path.exists('/disk2/support_files/archive/threeG_bsc.csv'):
        nsn_rnc=nsn_rnc[~nsn_rnc['Date'].isin(existing['Date'].unique())]
        nsn_rnc.to_csv('/disk2/support_files/archive/threeG_bsc.csv',header=False,index=False,mode='a')
    else:
        nsn_rnc.to_csv('/disk2/support_files/archive/threeG_bsc.csv',index=False,mode='a')

    pd.DataFrame(glob.glob(path1 + '/*.zip')).replace({'/home/ismayil/flask_dash/data/nokia/': '3G_'}, regex=True).\
    ...
    if len(all_files)>0:
        for filename in all_files:
            try: 
                if (filename.replace('/home/ismayil/flask_dash/data/nokia/','3G_util_') in existing_files.values): continue  
                files2 = zipfile.ZipFile(filename, 'r')
                df = pd.read_csv(files2.open(files2.namelist()[4]), sep=';')
                #if ('_37_00__' not in filename):
                #    df = df[df['PERIOD_START_TIME'].str.contains('00:00:00')]
                df['Date'] = pd.to_datetime(df['PERIOD_START_TIME'])
                df['HSDPA_stp_fails, BTS']=df['SETUP_FAIL_BTS_HS_DSCH_INT (M1002C416)']+df['SETUP_FAIL_BTS_HS_DSCH_STR (M1002C584)']+df['SETUP_FAIL_BTS_HS_DSCH_BGR (M1002C424)']
                df['HSDPA_stp_fails, MaxUsers']=df['DCH_SEL_MAX_HSDPA_USERS_BGR (M1002C476)']+df['DCH_SEL_MAX_HSDPA_USERS_INT (M1002C475)']+df['DCH_SEL_MAX_HSDPA_USERS_STR (M1002C591)']
                df['HSUPA_stp_fails, BTS']=df['SETUP_FAIL_EDCH_BTS_BGR (M1002C526)']+df['SETUP_FAIL_EDCH_BTS_INT (M1002C525)']+df['SETUP_FAIL_EDCH_BTS_STR (M1002C604)']
                df['HSUPA_stp_fails, BTS HW']=df['UL_DCH_SEL_BTS_HW_BGR (M1002C518)']+df['UL_DCH_SEL_BTS_HW_INT (M1002C517)']+df['UL_DCH_SEL_BTS_HW_STR (M1002C600)']
                df['HSUPA_stp_fails, MaxUsers']=df['UL_DCH_SEL_MAX_HSUPA_USR_BGR (M1002C516)']+df['UL_DCH_SEL_MAX_HSUPA_USR_INT (M1002C515)']+df['UL_DCH_SEL_MAX_HSUPA_USR_STR (M1002C599)']
                df=df[['Date','RNC name','WBTS name','WCEL name','HS_DSCH_CREDIT_RDCT_FRM_LOSS (M5000C178)','IUB_LOSS_CC_FRAME_LOSS_IND (M1022C71)',
                'Ovl TxCrPwr time share DL',
                'rab_stp_fail_cs_voice_ac (M1001C80)','rab_stp_fail_cs_voice_bts (M1001C81)','rab_stp_fail_cs_voice_frozbs (M1001C84)',
                'rab_stp_fail_cs_voice_rnc (M1001C83)','rab_stp_fail_cs_voice_trans (M1001C82)','rrc_conn_stp_fail_ac (M1001C3)',
                'rrc_conn_stp_fail_bts (M1001C4)','rrc_conn_stp_fail_frozbs (M1001C7)','rrc_conn_stp_fail_hc (M1001C2)','rrc_conn_stp_fail_rnc (M1001C6)',
                'rrc_conn_stp_fail_trans (M1001C5)','BTS_HSUPA_NO_HW_CAPA_DUR (M1000C270)','BTS_HSUPA_HW_LIMITED_DUR (M1000C269)',
                'HSDPA_stp_fails, BTS','HSDPA_stp_fails, MaxUsers','HSUPA_stp_fails, BTS','HSUPA_stp_fails, BTS HW','HSUPA_stp_fails, MaxUsers']]
                df.columns=['Date','RNC_name','WBTS_name','WCEL_name','HS_DSCH_CREDIT_RDCT_FRM_LOSS','IUB_LOSS_CC_FRAME_LOSS_IND',
                'Ovl_TxCrPwr_time_share_DL',
                'rab_stp_fail_cs_voice_ac (M1001C80)','rab_stp_fail_cs_voice_bts (M1001C81)','rab_stp_fail_cs_voice_frozbs (M1001C84)',
                'rab_stp_fail_cs_voice_rnc (M1001C83)','rab_stp_fail_cs_voice_trans (M1001C82)','rrc_conn_stp_fail_ac (M1001C3)',
                'rrc_conn_stp_fail_bts (M1001C4)','rrc_conn_stp_fail_frozbs (M1001C7)','rrc_conn_stp_fail_hc (M1001C2)','rrc_conn_stp_fail_rnc (M1001C6)',
                'rrc_conn_stp_fail_trans (M1001C5)','BTS_HSUPA_NO_HW_CAPA_DUR (M1000C270)','BTS_HSUPA_HW_LIMITED_DUR (M1000C269)',
                ...
                1
            try:
                df_site = pd.read_csv(files2.open(files2.namelist()[5]), sep=';')
                ...
                df_site['h_thrp_num']=(df_site['RECEIVED_HS_MACD_BITS (M5000C126)']-df_site['DISCARDED_HS_MACD_BITS (M5000C127)']+\
                    (df_site['MC_HSDPA_ORIG_DATA_PRI (M5002C128)']+df_site['MC_HSDPA_ORIG_DATA_SEC (M5002C129)'])*8)*500
                df_site['h_thrp_den']=df_site['HSDPA_BUFF_WITH_DATA_PER_TTI (M5000C85)']
                try:
                    ...
                    'Max HSDPA thr util ratio','h_thrp_num','h_thrp_den']]
                except:
                ...
                'MAX_AVAIL_R99_CE','Max_HSDPA_user_util_ratio','Max_HSUPA_thr_util_ratio','Max_HSDPA_thr_util_ratio','h_thrp_num','h_thrp_den']
                df_site[['LIC_HSDPA_THR','LIC_HSUPA_THR','LIC_NUM_HSDPA_USERS']].fillna(1,inplace=True)
            ...
            try:
                df_lcg = pd.read_csv(files2.open(files2.namelist()[6]), sep=';')
    ...
    if os.path.exists('/disk2/support_files/archive/threeG_util_cell.csv'):
        existing=pd.read_csv('/disk2/support_files/archive/threeG_util_cell.csv')
        util_cell=util_cell[~util_cell['Date'].isin(existing['Date'].unique())]        
        util_cell.to_csv('/disk2/support_files/archive/threeG_util_cell.csv',header=False,index=False,mode='a')
    else:
        util_cell.to_csv('/disk2/support_files/archive/threeG_util_cell.csv',index=False,mode='a')

    print('cell_util finished')

    if os.path.exists('/disk2/support_files/archive/threeG_util_site.csv'):
        existing=pd.read_csv('/disk2/support_files/archive/threeG_util_site.csv')
        util_site=util_site[~util_site['Date'].isin(existing['Date'].unique())]
        util_site.to_csv('/disk2/support_files/archive/threeG_util_site.csv',header=False,index=False,mode='a')
    else:
        util_site.to_csv('/disk2/support_files/archive/threeG_util_site.csv',index=False,mode='a')

    print('site_util finished')
    if os.path.exists('/disk2/support_files/archive/threeG_util_lcg.csv'):
        existing=pd.read_csv('/disk2/support_files/archive/threeG_util_lcg.csv')
        util_lcg=util_lcg[~util_lcg['Date'].isin(existing['Date'].unique())]
        util_lcg.to_csv('/disk2/support_files/archive/threeG_util_lcg.csv',header=False,index=False,mode='a')
    else:
        util_lcg.to_csv('/disk2/support_files/archive/threeG_util_lcg.csv',index=False,mode='a')

    print('lcg_util finished')
    print(time.time()-c1,'nokia_3G_util save finished')
    pd.DataFrame(glob.glob(path1 + '/*.zip')).replace({'/home/ismayil/flask_dash/data/nokia/': '3G_util_'}, regex=True).to_csv(os.path.join('/home/ismayil/flask_dash/data', 'files.csv'), mode='a', header=None, index=False)
    return 1
