...
main ='/mnt/raw_counters/Corporate Folder/CTO/Technology Governance and Central Support/RAN QA/Daily/Raw_counters/Core/'
for num,i in enumerate(files):
      try:
            ...
            new_mode=df[(df['mode']=='Gb mode') & (df['Site'].str.contains('@'))].groupby('Date').sum()
            new_mode['lte_bkc_subs']=db['S1 mode max attached user'].values
            new_mode['lte_azf_subs']=da['S1 mode max attached user'].values
            ...
            new_mode['lte_inroam_subs']=dc['S1 mode max attached user'].values
            new_mode['tot_2G']=df[df['mode']=='Gb mode'].groupby('Date').sum()['Max attach user'].values+user_t.groupby('Date').sum()['Gb mode max attached user'].values
            new_mode['nn']=user_t.groupby('Date').sum()['S1 mode max attached user'].values
            new_mode['nn2']=user_t.groupby('Date').sum()['Gb mode max attached user'].values
            new_mode['nn3']=user_t.groupby('Date').sum()['Iu mode max attached user'].values
            b['lte_inroam']=max(new_mode['inroamer']+new_mode['lte_inroam_subs'])
            b['tot_2G']=round(max(df[(df['mode']=='Gb mode') & (df['Site'].str.contains('@'))].groupby('Date').sum()['Max attach user'].values+\
                        user_t.groupby('Date').sum()['Gb mode max attached user'].values),0)
            b['tot_3G']=round(max(df[(df['mode']=='Iu mode') & (df['Site'].str.contains('@'))].groupby('Date').sum()['Max attach user'].values+\
                        user_t.groupby('Date').sum()['Iu mode max attached user'].values),0)
            ...
            print(i)
            continue
pd.concat(h).to_csv('/mnt/raw_counters/Corporate Folder/CTO/Technology Governance and Central Support/Core QA/scheduled_reports/users_'+files2[-1][4:-4]+'.csv',index=False)
pd.concat(h2).to_csv('/mnt/raw_counters/Corporate Folder/CTO/Technology Governance and Central Support/Core QA/scheduled_reports/users_cmm_'+files2[-1][4:-4]+'.csv',index=False)
