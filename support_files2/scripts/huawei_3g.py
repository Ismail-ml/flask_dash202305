...
def run(path1,path2,path3,tracker):
        ...
        for filename in all_files:
            ...
            df=df[['Object Name','Result Time','cs_inter_freq_ho_num',
                ...
                'hsdpa_sr_den','hsupa_sr_num','hsupa_sr_den','voice_sr_num2','voice_sr_den2','r99_sr_num','r99_sr_den',
                       ...
                       'voice_sr_den1','voice_dr_num','voice_dr_den','ps_ac_sr_num1','ps_ac_sr_den1','ps_ac_sr_num2',
    ...
    def proccess_excel2(diddd):
        ...
        for filename in all_files:
            ...
            df['rtwp_num'] = 10 ** (-11.0) * df['73426885'] + 10 ** (-10.75) * df['73426886'] + 10 ** (-10.65) * df['73426887'] +10 ** (-10.55) * df['73426888'] + 10 ** (-10.45) * df['73426890'] + 10 ** (-10.35) * df['73426891'] +10 ** (-10.25) * df['73426892'] + 10 ** (-10.15) * df['73426893'] + 10 ** (-10.05) * df['73426894'] +10 ** (-09.95) * df['73426895'] + 10 ** (-09.85) * df['73426896'] + 10 ** (-09.70) * df['73426897'] +10 ** (-09.50) * df['73426898'] + 10 ** (-09.30) * df['73426899'] + 10 ** (-09.05) * df['73426900'] +10 ** (-08.75) * df['73426901'] + 10 ** (-08.45) * df['73426902'] + 10 ** (-08.15) * df['73426903'] +10 ** (-07.75) * df['73426904'] + 10 ** (-07.25) * df['73426905'] + 10 ** (-06.75) * df['73426906'] + 10 ** (-06.50) * df['73426907']
            df['rtwp_den'] = df['73426885'] + df['73426886'] + df['73426887'] + df['73426888'] + df['73426890'] + df['73426891'] +df['73426892'] + df['73426893'] + df['73426894'] + df['73426895'] + df['73426896'] + df['73426897'] + df['73426898'] +df['73426899'] + df['73426900'] + df['73426901'] + df['73426902'] + df['73426903'] + df['73426904'] + df['73426905'] +df['73426906'] + df['73426907']
            df['cs_traf']=df['73426291']
            ...
            df=df[['Object Name','Result Time','cell_avail_num',
                ...
                'r99_dr_den','cs_traf','73422167','73422161','73422168','73422163','rtwp_num','rtwp_den']]
            df['Date']=pd.to_datetime(df['Result Time'], format='%Y-%m-%d %H:%M')
            df['RNC_name']=df['Object Name'].apply(lambda x: x[:6])
            df['Cell_name']=df['Object Name'].apply(lambda x: re.search(r'\w{4}\d{4}\w',x).group())
            #df['Site_name']=df['Cell_name'].apply(lambda x: x[:8])
    ...
    # burda ishleyirdi hw_rnc.drop(columns=['rtwp_num','rtwp_den']).to_hdf(r'/disk2/support_files/archive/combined_bsc.h5', '/threeG',
    #              append=True,
    #              format='table', data_columns=['Date', 'RNC_name', 'Vendor', 'Region'], complevel=5,
    #              min_itemsize={'RNC_name': 10, 'Vendor': 10, 'Region': 15})

    p=datetime.datetime.strftime(datetime.datetime.now(),'%d.%m.%Y_%H')
    #hw_rnc.to_csv('/mnt/raw_counters/Corporate Folder/CTO/Technology Governance and Central Support/RAN QA/Daily/Radio_Main_KPIs/3G_hw_'+p+'.csv',
    #                        index=False)


    df3=hw_rnc.drop(columns=['rtwp_num','rtwp_den'])
    ...
    df3.to_csv('/disk2/support_files/atoti/bi/3G_huawei.csv',index=False) #' + str(datetime.datetime.now()) + '
