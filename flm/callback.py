...
lb = {'margin':dict(l=5, r=10, t=30, b=15),'height':250,'template': 'simple_white'}
...
def register_callback(dashapp):
    ...
    def display_output(val,reg):
        ...
        df=pd.read_csv('/home/ismayil/flask_dash/data/active_alarms/active_alarms.csv')
        df2=df[df['time']==val]
        ...
        down_sorted=down_sorted.append(pd.DataFrame(data=[['Absheron'],['Aran'],['Baku'],['Ganja'],['Lankaran'],['Naxchivan'],['Qarabag'],['Quba'],['Sheki']],columns=['location']),ignore_index=True)
        down_sorted=down_sorted[['location','Unique down','MPF start','PG start','SG start']].groupby('location',as_index=False).sum().sort_values(by='location')
        down_sorted2=df[['location','admin_region','Unique down','MPF start','PG start','SG start']].sort_values(by='location')
        down_sorted['status'] = down_sorted['Unique down'].apply(lambda x: '🔥' if x > 30 else 
                                                                                ('🚒' if x > 20 else (
                                                                                    '⚠️' if x > 10 else '🟢')))
        ...
        if len(df_site)>0:
            ...
            fig10 =df_site2[['SITE_ID','Site Name','Economical Region','Down_duration']].\
                rename(columns={'Economical Region':'Region'}).to_dict('records')
            if reg=='All Regions':
                zoom=6.5
            else:
                zoom=8.5
            ...
            figure.update_layout(title=
                                    go.layout.Title(text='<b>' + str(val) + ' values for '+str(reg)+# + str(
            ...
            figure = html.H1("No down in "+str(reg), className="card-title",style={'height': 500})
            fig10=[]
        ...
        fig2.update_layout({'title':str(reg)+' - Cell Availability, %'})
